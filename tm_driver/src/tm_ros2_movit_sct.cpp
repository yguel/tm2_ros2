                                                         ...
                                                         std::shared_ptr<const control_msgs::action::FollowJointTrajectory::Goal> goal)
{
  ...
  if (!is_fake_)
  {
    if (!svr_.is_connected())
    {
      ...
      print_info("[ACTION] TM_ROS: TMFlow server (TmSvrCommunication) is not connected");
      return rclcpp_action::GoalResponse::REJECT;
    }
    if (!sct_.is_connected())
    {
      print_info("[ACTION] -----GOAL REJECT-----");
      print_info("[ACTION] TM_ROS: TMFlow (TmSctCommunication) is not connected");
      return rclcpp_action::GoalResponse::REJECT;
    }
    if (state_.has_error())
    {
      print_info("[ACTION] -----GOAL REJECT-----");
      print_info("[ACTION] TM_ROS: Robot state has error");
      return rclcpp_action::GoalResponse::REJECT;
    }
  }
  ...
  {
    print_info("[ACTION] -----GOAL REJECT-----");
    print_info("[ACTION] TM_ROS: Trajectory has no points");
    return rclcpp_action::GoalResponse::REJECT;
  }
