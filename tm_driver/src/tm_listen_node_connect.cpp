...
ListenNodeConnection::ListenNodeConnection(TmDriver &iface, std::function<void(TmSctData)> sct_msg, std::function<void(std::string, std::string)> sta_msg, bool is_fake_)
...
bool ListenNodeConnection::send_data()
{
    ...
    {
        print_info("TM_ROS: (Listen node) recv_spin_once error");
        print_info("rc == ERR : %d", rc == TmCommRC::ERR);
        print_info("rc == NOTREADY : %d", rc == TmCommRC::NOTREADY);
        print_info("rc == NOTCONNECT : %d", rc == TmCommRC::NOTCONNECT);
        return false;
    }
    else if (rc == TmCommRC::TIMEOUT)
    {
        return true;
    }
    else if (rc != TmCommRC::OK)
    {
        print_info("TM_ROS: (Listen node) recv_spin_once not ok, return code is %d", rc);
    }
