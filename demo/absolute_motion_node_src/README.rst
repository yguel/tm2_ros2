...
There is an action server to move the robot in an absolute position.

To launch the action server, you can use the following command:

.. code-block:: bash

   export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && ros2_humble_src && ros2 run demo absolute_motion_node

To use the action server, you can call it with the following command:

.. code-block:: bash

   export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && ros2_humble_src && ros2 action send_goal /go_to_absolute_position tm_msgs/action/GoToPosition "{x: 0.00, y: 30., z: 0., rx_deg: 0., ry_deg: 0., rz_deg: 0., speed_percent: 20, time_to_top_speed_ms: 300, frame: LASER_ROS2}"
...
.. code-block:: bash
   ...
   export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && ros2_humble_src && ros2 action cancel /go_to_absolute_position